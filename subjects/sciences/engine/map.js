/* Learning Atlas — Mathematics · learning map (focused-unit model).
   ...
   grade is in focus; this stays a pure view over SCI.Graph + the progress store.

   Exposes window.SCI.Map: mount(host, opts) -> {
...
(function () {
  ...
  const U = SCI.util, Graph = SCI.Graph, Store = SCI.Store;
  ...
  function mount(host, opts) {
    ...
    // Sciences opens on the COURSE PICKER (overview): the grade cards plus the
    // extra courses passed in opts.courses (e.g. the kept HS Biology explorer and
    // the still-to-come Chemistry / Physics courses). Picking a grade drills in.
    let mode = 'overview';
    ...
    /* ---- overview = the COURSE PICKER: each open grade as a progress card,
       then the "other courses" strip (kept HS Biology explorer + coming-soon
       high-school courses) passed in via opts.courses. --------------------- */
    function renderOverview() {
      const gradeCards = grades().map(g => {
        ...
        const strands = Array.from(new Set(cs.map(c => c.strand))).slice(0, 4).join(' · ');
        // Nothing is grade-locked in Sciences, so a grade with concepts but no
        // started/authored lessons reads "Explore" (coming-soon nodes are still
        // browsable), never "Locked".
        const state = gLocked ? 'locked' : mastered === total && total ? 'done' : available || mastered ? 'now' : 'open';
        const badge = state === 'done' ? '<span class="badge badge-good">Complete</span>'
          : state === 'now' ? '<span class="badge badge-accent">In progress</span>'
            : state === 'open' ? '<span class="badge">Explore</span>'
        ...
        return `<button class="m-ucard" data-grade="${g}">
          <div class="m-ucard-top"><h3>${gradeLabel(g)} Science</h3>${badge}</div>
      ...
      const courseCards = courses.map((c, i) => {
        ...
        const badge = soon ? '<span class="badge">Coming soon</span>' : '<span class="badge badge-good">Explore</span>';
        return `<button class="m-ucard m-ucard-course${soon ? ' soon' : ''}" data-course="${i}"${soon ? ' aria-disabled="true"' : ''}>
          <div class="m-ucard-top"><h3>${c.name}</h3>${badge}</div>
          <div class="m-ucard-meta">${c.meta || ''}</div>
          ${c.desc ? `<p class="m-ucard-desc">${c.desc}</p>` : ''}
          <div class="m-ucard-foot"><span class="m-ucard-go">${soon ? 'In planning' : 'Open explorer →'}</span></div>
        </button>`;
      }).join('');

      overviewEl.innerHTML = `
        <div class="m-picker-intro">
          <h2>Choose a course</h2>
          <p>Indiana-aligned science by grade, plus the interactive explorers. Grades 3, 5 and 7 follow the Indiana Academic Standards for Science (2023).</p>
        </div>
        <div class="m-picker-sec"><span class="m-picker-lab">Grade-level science</span><div class="m-ucards">${gradeCards}</div></div>
        ${courseCards ? `<div class="m-picker-sec"><span class="m-picker-lab">Explorers &amp; high-school courses</span><div class="m-ucards">${courseCards}</div></div>` : ''}`;

      overviewEl.querySelectorAll('[data-grade]').forEach(b => b.onclick = () => { focus = b.dataset.grade; setMode('focus'); });
      overviewEl.querySelectorAll('[data-course]').forEach(b => {
        const c = courses[+b.dataset.course];
        if (c && c.status === 'live' && c.href) b.onclick = () => { location.href = c.href; };
      });
    }
