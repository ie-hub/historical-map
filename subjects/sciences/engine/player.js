/* Learning Atlas — Mathematics · lesson player.
   ...
   3 = also cleared the Challenge step. Exposes window.SCI.Player.             */
(function () {
  const SCI = (window.SCI = window.SCI || {});
  const U = SCI.util, Store = SCI.Store, Graph = SCI.Graph;
  ...
  function Player(host, opts) {
    ...
    function renderStep() {
      ...
      const def = SCI.Components.get(step.component);
    ...
    function renderUnlock(card) {
      ...
      const stds = (concept.standards || []).map(code => SCI.Standards && SCI.Standards.get(code)).filter(Boolean);
  ...
  SCI.Player = { register, get, Player, KIND_LABEL, KIND_ICON };
})();
