/* Learning Atlas — Mathematics · reusable interactive components.
   ...
   with — never a wall of text. One contract:
     ...
     SCI.Components.register(name, { title, mount(host, config, ctx) })
   ...
   mount() may return { destroy() } for cleanup. Exposes window.SCI.Components. */
(function () {
  ...
  const R = SCI.Components.register.bind(SCI.Components);

  /* ===================================================================== SCIENCE
     Manipulatives shared with the Life Explorer engine — classify, sequence,
     match and run a live model. Same ctx contract as the math components above,
     so they drop straight into the 9-stage lesson flow. Science leans on these
     (NGSS is "develop a model", "construct an argument") more than on numeric
     entry, so they carry most of the interactive weight in the grade lessons. */

  /* --- sortCards: drop labelled cards into the group they belong in.
  ...
  R('sortCards', { title: 'Sort into groups', mount(host, cfg, ctx) {
    ...
    binRow.querySelectorAll('.m-sc-bin').forEach(binEl => {
      binEl.onclick = () => {
        ...
        ctx.attempt(ok, ok ? null : { misconception: it.misconception }); ctx.count('sortCards');
        if (ok) {
  ...
  /* --- orderList: tap items into the correct sequence.
  ...
  R('orderList', { title: 'Put in order', mount(host, cfg, ctx) {
    ...
    function drawChain() {
      chain.innerHTML = correct.slice(0, next).map((l) => `<span class="m-ol-slot filled">${l}</span>`).join(`<span class="m-ol-arrow">${arrow}</span>`);
    }
    function drawPool() {
      poolEl.innerHTML = '';
      pool.forEach((it, k) => {
  ...
  /* --- matchPairs: match a term to its description. config: { prompt, pairs:[{left,right}] } */
  ...
  /* --- simSlider: change a variable, watch the model respond; a goal must hold.
     config: { prompt, vars:[{id,label,min,max,step,value,unit}],
       outputs:[{id,label,unit,fn(v),max,color}], viz?(v,out), goal:{text,done,test(v,out)} } */
  R('simSlider', { title: 'Run the model', mount(host, cfg, ctx) {
    const vars = cfg.vars || [];
