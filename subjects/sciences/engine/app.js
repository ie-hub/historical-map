/* Learning Atlas — Sciences · app boot (sidebar shell).
   The container subject: it opens on a COURSE PICKER (the map's overview) listing
   the Indiana-aligned grade courses (3, 5, 7) plus the kept high-school explorers
   (Biology). Composes the shared shell: a left curriculum sidebar (built from the graph),
   ...
   Atlas.wireShell).                                                            */
(function () {
  const SCI = window.SCI, Graph = SCI.Graph, Store = SCI.Store;
  ...
  function crumbsForMap() {
    ...
    const subject = { label: 'Sciences', on: () => mapView.setMode('overview') };
    if (mapView.mode() === 'overview') setCrumbs([atlas, subject, { label: 'Courses' }]);
  ...
  function openConcept(id) {
    ...
    const stds = (c.standards || []).map(code => SCI.Standards && SCI.Standards.get(code)).filter(Boolean);
    ...
    const body = `
      ...
      ${st === 'locked' ? (gLocked
        ? `<div class="alert alert-warn" style="margin-top:16px"><span class="alert-ic">🔒</span><span><b>${gradeLabel(c.grade)}</b> is locked while it's being built. Grades 3, 5 and 7 are open right now.</span></div>`
  ...
  function renderProgress() {
    ...
    const fav = s.favourite ? (SCI.Components.get(s.favourite) || {}).title || s.favourite : null;
    const body = `
    ...
    rail.update({ title: 'Your progress', kind: 'Explorer', tabs: [], body, onBody(elm) {
      elm.querySelectorAll('[data-go]').forEach(b => b.onclick = () => openConcept(b.dataset.go));
      const rst = elm.querySelector('[data-reset]'); if (rst) rst.onclick = () => { if (confirm('Reset all science progress? This cannot be undone.')) { Store.reset(); openProgress(); } };
    } });
  }
  function titles(s) {
    return [
      { icon: '🌱', name: 'First Steps', req: 'Finish 1 lesson', earned: s.completed >= 1 },
      { icon: '🔬', name: 'Investigator', req: 'Master 3 concepts', earned: s.mastered >= 3 },
      { icon: '🔥', name: 'On a Streak', req: '3-day streak', earned: s.streak >= 3 },
      { icon: '⭐', name: 'Star Collector', req: 'Earn 15 stars', earned: s.stars >= 15 },
      { icon: '🎓', name: 'Young Scientist', req: 'Master 10 concepts', earned: s.mastered >= 10 }
    ];
  }
  ...
  function renderStandards() {
    ...
    const cov = SCI.Standards.coverage(grade);
    const src = SCI.Standards.SOURCE;
    let body;
    if (!cov.total) {
      body = `<div class="empty"><div class="empty-ic">🗺️</div><p>Standards for ${gradeLabel(grade)} haven't been mapped yet.</p>
        <p class="ui" style="font-size:13px;color:var(--muted)">Pick a grade course from the overview to see its Indiana coverage.</p></div>`;
    } else {
  ...
  function boot() {
    ...
    player = SCI.Player.Player(el('m-lesson'), { onExit: showMap, onComplete: () => { /* store change re-syncs */ } });
    mapView = SCI.Map.mount(el('m-map'), {
