/* Learning Atlas — Mathematics · knowledge graph.
   ...
   Register with SCI.Graph.add(concept). Status is derived, never stored:
...
(function () {
  ...
  const OPEN_GRADES = null;   // all built grades (3, 5, 7) are open
  ...
  /* "Unlock all". When on, every concept's prerequisites are treated as met, so
     ...
     with SCI.Graph.setUnlockAll(false); the choice is persisted in localStorage. */
  function readUnlockFlag() {
    try {
      if (/[?&#]lock\b/i.test(location.href)) { localStorage.setItem('atlas.sci.unlockAll', '0'); return false; }
      if (/[?&#]unlock(all)?\b/i.test(location.href)) { localStorage.setItem('atlas.sci.unlockAll', '1'); return true; }
      return localStorage.getItem('atlas.sci.unlockAll') !== '0';   // default: unlocked
    } catch (e) { return !/[?&#]lock\b/i.test(location.href || ''); }
  }
  let UNLOCK_ALL = readUnlockFlag();
  function setUnlockAll(on) {
    UNLOCK_ALL = !!on;
    try { localStorage.setItem('atlas.sci.unlockAll', on ? '1' : '0'); } catch (e) {}
  ...
  SCI.Graph = { GRADES, add, get, all, status, prereqsMet, gradeLocked, unlockedBy, children, recommended, remediationFor, search, setUnlockAll };
})();
