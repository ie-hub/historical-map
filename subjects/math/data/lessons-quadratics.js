/* Learning Atlas — Mathematics · Algebra I lessons: the quadratic & exponential
   strand (AI.QE) plus its factoring prerequisite (AI.NE.4). The parabolaExplorer
   manipulative is the teaching tool: match parabolas in vertex form, click
   vertices, click zeros — so factors ↔ zeros ↔ x-intercepts is something the
   learner DOES, not memorises. Summative checks tag their distractors with the
...
(function () {
  ...
  reg({
    ...
    steps: [
      ...
      { kind: 'explore', title: 'Find the pair', intro: 'Factoring x² + bx + c means finding two numbers that MULTIPLY to c and ADD to b.', component: 'problemSet',
        config: { problems: [
      ...
      { kind: 'discover', title: 'Factoring is un-multiplying', text: 'To factor <b>x² + bx + c</b>, find two numbers with product <b>c</b> and sum <b>b</b>: x² + 5x + 6 = <b>(x + 2)(x + 3)</b>. Two specials worth knowing on sight: the <b>difference of squares</b> x² − 9 = (x + 3)(x − 3), and the <b>perfect square</b> x² + 6x + 9 = (x + 3)².', rule: 'product = c · sum = b' },
      ...
      { kind: 'extend', title: 'Go further', intro: 'Push the un-multiplying idea.',
        items: [
          { icon: '🧱', label: 'Area model', detail: 'Draw x² + 5x + 6 as a rectangle of tiles: one x², five x-strips, six units. Its side lengths ARE the factors.' },
  ...
  reg({
    ...
    steps: [
      ...
      { kind: 'discover', title: 'Three ways in', text: 'Pick the tool that fits the form: bare square → <b>square roots</b> (x² = 49 → x = ±7 — never forget the negative). Factorable → <b>factor and use the zero product</b>: x² − x − 6 = (x − 3)(x + 2) = 0 → x = 3 or −2. Anything else → the <b>quadratic formula</b>, x = (−b ± √(b² − 4ac)) / 2a, which always works.', rule: 'square roots · factoring · the formula — match the tool to the form' },
      ...
      { kind: 'extend', title: 'Go further', intro: 'Beyond factorable.',
        items: [
          { icon: '🧮', label: 'Meet the formula', detail: 'Use x = (−b ± √(b² − 4ac))/2a on x² − 2x − 8 = 0. Do you get 4 and −2, matching the factoring?' },
          { icon: '🚀', label: 'A real projectile', detail: 'h = −5t² + 20t gives a rocket’s height. When does it land (h = 0)? Factor out t.' },
          { icon: '🔍', label: 'The discriminant', detail: 'The part under the root, b² − 4ac, predicts how many real solutions exist. Test it on x² + 1 = 0.' }
        ] }
    ]
  });
